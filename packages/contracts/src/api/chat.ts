...
export type SlideFeedbackKind = 'comment' | 'design-token' | 'inspect-edit' | 'semantic-edit';
export type SlideFeedbackStatus = 'queued' | 'applying' | 'needs_review' | 'applied' | 'dismissed' | 'failed';

export interface ChatSlideFeedbackAttachment {
  id: string;
  order: number;
  kind: SlideFeedbackKind;
  slideId: string;
  pageIndex?: number;
  line?: number;
  column?: number;
...
export interface ChatVaultContextAttachment {
  slug: string;
  title: string;
...
export interface SlideFeedback extends ChatSlideFeedbackAttachment {
  projectId: string;
  conversationId?: string;
