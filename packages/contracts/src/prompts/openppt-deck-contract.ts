export const OPENPPT_DECK_CONTRACT = `# OpenPPT Deck Contract

This project is an OpenPPT Web-PPT deck. The canonical editable source is Open Slide TSX, not an HTML deck artifact.
...
- Do not leave OpenPPT fallback tokens such as \`#0f172a\`, \`#38bdf8\`, \`Inter\`, or generic \`system-ui\` stacks when a selected Vault template specifies different colors or typefaces.
- Motion is Vault-first: if the selected Vault context says no entrance animation, instant state changes, fade-only, or source-observed motion only, follow that over the generic OpenPPT motion helpers.
...
- Motion coverage is a readiness gate, not decoration. When motion is allowed or requested, at least 60% of pages must expose applied motion via \`data-osd-motion-id\`, \`motionAttrs(...)\`, or OpenPPT helper classes such as \`os-motion\`, \`os-fade-up\`, \`os-line-grow\`, \`os-canvas-swap\`, or \`os-motion-stagger\`. Do not satisfy this by defining unused helpers.
...
Delivery priority:
1. Editable Web-PPT preview/editing inside OpenPPT.
