...
import { MEDIA_GENERATION_CONTRACT, OPENPPT_DECK_MEDIA_CONTRACT } from './media-contract';
import { OPENPPT_DECK_CONTRACT } from './openppt-deck-contract';
...
export function composeSystemPrompt({
  ...
  const metaBlock = renderMetadataBlock(metadata, template, vaultTemplateBody, vaultCatalogBody);
...
function renderMetadataBlock(
  ...
  if (metadata.kind === 'deck') {
    ...
    lines.push(`- **slideId**: ${metadata.slideId ?? '(unknown - use slides/<project-slug>/index.tsx)'}`);
    lines.push(`- **slideWorkspace**: ${metadata.slideWorkspace ?? 'slides'}`);
    ...
    if (metadata.vaultTemplate) {
      ...
      lines.push(`- **vaultTemplate**: ${vault.title ?? 'Untitled Vault template'} (${vault.slug})`);
      ...
      if (vaultTemplateBody && vaultTemplateBody.trim().length > 0) {
        lines.push('');
        lines.push('### Active Vault `open-slide-theme.md`');
        lines.push(
          'This theme guidance is authoritative for the OpenPPT deck. Bind its palette, typography, layout, component language, and anti-patterns into `slides/<slideId>/index.tsx`.',
        );
    ...
    lines.push(
      '- **canonicalSource**: Open Slide TSX under `slides/<slideId>/index.tsx`. Do not output the legacy HTML deck artifact for OpenPPT deck projects.',
    );
  }
...
function renderVaultComponentMotionRecipesBlock(
  ...
  lines.push(
    'Motion implementation rules: in generated OpenPPT TSX, use `MotionStyles`, `motionFromRecipe`, `motionAttrs`, and `data-osd-motion-id` for recipe-backed components; CSS motion must include a `prefers-reduced-motion` fallback; keep `data-osd-motion-id` in the DOM so PPTX export can capture the recipe metadata.',
  );
  return lines;
}
