 ...
 */
import type { AgentEvent, ChatCommentAttachment, ChatMessage, ChatSlideFeedbackAttachment, ChatVaultContextAttachment } from '../types';
import type {
...
export interface DaemonStreamOptions {
  ...
  slideFeedbackAttachments?: ChatSlideFeedbackAttachment[];
...
export async function streamViaDaemon({
  ...
  const request: ChatRequest = {
    ...
    slideFeedbackAttachments: slideFeedbackAttachments ?? [],
    vaultContextAttachments: vaultContextAttachments ?? [],
    model: model ?? null,
...
async function consumeDaemonRun({
  ...
  try {
      ...
      while (true) {
        ...
        while ((idx = buf.indexOf('\n\n')) !== -1) {
          ...
          if (event.event === 'agent') {
            ...
            if (translated.kind === 'raw') {
              const structuredError = extractStructuredAgentError(translated.line);
              if (structuredError && !structuredAgentErrorBuf.includes(structuredError)) {
                structuredAgentErrorBuf += `${structuredAgentErrorBuf ? '\n' : ''}${structuredError}`;
              }
            }
    ...
    if (endStatus === 'failed' || exitSignal || (exitCode !== null && exitCode !== 0)) {
      const structuredTail = structuredAgentErrorBuf.trim().slice(-800);
      const stderrTail = stderrBuf.trim().slice(-400);
      const detail = structuredTail
        ? `${structuredTail}${stderrTail ? `\n\nstderr:\n${stderrTail}` : ''}`
        : stderrTail;
      handlers.onError(
        new Error(`agent exited with ${exitSignal ? `signal ${exitSignal}` : `code ${exitCode}`}${detail ? `\n${detail}` : ''}`),
      );
      return;
    }
    handlers.onDone(acc);
...
function readStructuredErrorMessage(value: Record<string, unknown>): string | null {
  const directMessage = typeof value.message === 'string' ? value.message : null;
  if (directMessage) {
    const embedded = parseJsonObject(directMessage);
    const embeddedMessage = embedded ? readStructuredErrorMessage(embedded) : null;
    if (embeddedMessage) return embeddedMessage;
  }

  const nestedError = value.error;
  if (nestedError && typeof nestedError === 'object' && !Array.isArray(nestedError)) {
    const nestedMessage = readStructuredErrorMessage(nestedError as Record<string, unknown>);
...
function extractStructuredAgentError(line: string): string | null {
  const parsed = parseJsonObject(line);
  if (!parsed) return null;
  if (parsed.type !== 'error' && parsed.event !== 'error' && !parsed.error) return null;
  return readStructuredErrorMessage(parsed);
}
