...
import { fetchAppUpdateCheck, fetchCodexImageProxyStatus } from '../providers/registry';
import {
  FAILURE_SOUNDS,
  SUCCESS_SOUNDS,
...
const CODEX_IMAGE_PROXY_FALLBACK_BASE_URL = 'http://127.0.0.1:<daemon-port>/v1';

export function codexImageProxyAuthLabel(status: CodexImageProxyStatus | null): string {
  if (!status) return 'Checking...';
  if (!status.auth.configured) return 'No Codex / Hermes login';
  if (status.auth.source === 'oauth-hermes') return 'Hermes OAuth';
...
function appUpdateStatusText(
  ...
  if (!update) return '进入关于页后会自动检查更新。';
  if (update.status === 'available') return `发现新版本 ${update.latestVersion ?? ''}`.trim();
  if (update.status === 'latest') return `已是最新版 ${update.current.version}`;
  if (update.status === 'disabled') return '更新检查未配置。';
  return update.error ? `检查失败：${update.error}` : '检查失败。';
}

function appUpdateActionUrl(update: AppUpdateCheckResponse | null): string | null {
  return update?.asset?.url ?? update?.releaseUrl ?? update?.notesUrl ?? null;
}

export function buildCodexImageProxyCurl(baseUrl: string, model = 'gpt-image-2'): string {
  const normalized = (baseUrl || CODEX_IMAGE_PROXY_FALLBACK_BASE_URL).replace(/\/+$/, '');
  return `curl ${normalized}/images/generations -H 'content-type: application/json' -d '{"model":"${model}","prompt":"A clean product poster","size":"1024x1024"}'`;
}
...
export function SettingsDialog({
  ...
  const [appUpdateCheck, setAppUpdateCheck] = useState<AppUpdateCheckResponse | null>(null);
  ...
  const runAppUpdateCheck = useCallback(async () => {
    ...
    setAppUpdateError('');
    const next = await fetchAppUpdateCheck();
    if (next) {
      setAppUpdateCheck(next);
    } else {
      setAppUpdateError('无法连接更新检查服务。');
    }
  ...
  useEffect(() => {
    if (activeSection !== 'about' || !appVersionInfo || updateAutoCheckedRef.current) return;
    updateAutoCheckedRef.current = true;
    void runAppUpdateCheck();
  }, [activeSection, appVersionInfo, runAppUpdateCheck]);
  ...
  const appUpdateUrl = appUpdateActionUrl(appUpdateCheck);
  const appUpdateText = appUpdateStatusText(appUpdateCheck, appUpdateChecking, appUpdateError);
            ...
            <section className="settings-section">
              ...
              {appVersionInfo ? (
                <>
                  ...
                  <div className={`settings-update-card status-${appUpdateCheck?.status ?? 'idle'}`}>
                    <div className="settings-update-head">
                      <div>
                        <strong>软件更新</strong>
                        <small>自动检查新版本；源码运行时只提示，不覆盖本地文件。</small>
                      </div>
                      <button
                        type="button"
                        className="ghost settings-update-button"
                        disabled={appUpdateChecking}
                        onClick={runAppUpdateCheck}
                      >
                        <Icon name={appUpdateChecking ? 'spinner' : 'refresh'} size={14} />
                        <span>{appUpdateChecking ? '检查中' : '检查更新'}</span>
                      </button>
                    </div>
                    <p>{appUpdateText}</p>
                    {appUpdateCheck?.notes ? <p className="settings-update-notes">{appUpdateCheck.notes}</p> : null}
                    {appUpdateCheck?.sourceMode === 'source' ? (
                      <p className="settings-update-hint">
                        源码版建议确认本地改动后执行 <code>git pull &amp;&amp; pnpm install</code>。
                      </p>
                    ) : null}
                    {appUpdateCheck?.status === 'available' && appUpdateUrl ? (
                      <a
...
type CodexImageProxyCopyTarget = 'baseUrl' | 'curl';

function CodexImageProxyCard() {
  const [status, setStatus] = useState<CodexImageProxyStatus | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [copied, setCopied] = useState<CodexImageProxyCopyTarget | null>(null);
  ...
  useEffect(() => {
    ...
    fetchCodexImageProxyStatus()
      .then((next) => {
        if (cancelled) return;
        setStatus(next);
        setError(next ? '' : 'Status unavailable');
      })
      .catch((err) => {
        if (cancelled) return;
        setStatus(null);
  ...
  const refresh = async () => {
    setLoading(true);
    setError('');
    const next = await fetchCodexImageProxyStatus();
    setStatus(next);
    setError(next ? '' : 'Status unavailable');
    setLoading(false);
  };

  const baseUrl = status?.baseUrl ?? CODEX_IMAGE_PROXY_FALLBACK_BASE_URL;
  const model = status?.defaultModel ?? 'gpt-image-2';
  const curl = buildCodexImageProxyCurl(baseUrl, model);
  const authLabel = codexImageProxyAuthLabel(status);
  const accountLabel = status
    ? status.auth.accountIdConfigured
      ? `...${status.auth.accountIdTail}`
      : 'Not found'
    : 'Checking...';
  ...
  const markCopied = (target: CodexImageProxyCopyTarget) => {
    setCopied(target);
    if (copyTimerRef.current) clearTimeout(copyTimerRef.current);
  ...
  const copyBaseUrl = async () => {
    if (await copyText(baseUrl)) markCopied('baseUrl');
  };
  const copyCurl = async () => {
    if (await copyText(curl)) markCopied('curl');
  };
...
// and Linux ship a system /usr/bin/od (octal-dump) that shadows any
  // `od` we might add to PATH, and most Slides from Anything users run from
...
// a non-ASCII home directory path trips that and throws
      ...
      <div className="section-head">
        <div>
          ...
          <p className="hint">
            ...
            VS Code, Antigravity, Zed, Windsurf) read your Slides from Anything
      ...
      <div className="settings-about-list" style={{ display: 'block' }}>
        {infoError ? (
          ...
          >
            ...
            ({infoError}). Make sure Slides from Anything is running, then reopen this
            panel.
          </div>
        ) : null}
        ...
        <div style={{ marginTop: 20, lineHeight: 1.55 }}>
          ...
          >
            ...
            <li>
              Default to the project and file you have open in Slides from Anything,
        ...
        >
          Slides from Anything must be running for MCP tool calls to succeed. If
          you started your coding agent before opening Slides from Anything,
