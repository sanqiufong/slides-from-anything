...
export function NewProjectPanel({
  ...
  const skillIdForTab = useMemo(() => {
    ...
    if (tab === 'deck') {
      ...
      return list.find((s) => s.id === 'openppt-deck')?.id
  ...
  const canCreate =
    ...
    (!showDesignSystemPicker || !loading);
...
function VaultTemplatePicker({
  ...
  return (
    <div className="newproj-section vault-template-picker">
      <div className="vault-template-head">
        <label className="newproj-label">Design Vault templates</label>
        <button
          type="button"
          className="ghost vault-template-refresh"
          disabled={loading}
          onClick={onRefresh}
        >
      ...
      <input
        ...
        placeholder="Search Vault templates"
        value={query}
        onChange={(event) => onQuery(event.target.value)}
      />
      <div className="template-list vault-template-list">
        <BlankVaultTemplateCard
          active={selectedSlug === null}
          onSelect={() => onSelect(null)}
        />
      </div>
      {loading ? (
        <Skeleton height={82} width="100%" radius={8} />
      ) : filtered.length === 0 ? (
        <div className="template-howto">
          <span className="template-howto-title">
            {error ?? 'No Vault templates found'}
          </span>
          <span className="template-howto-body">
            {query.trim()
              ? 'Clear the search or refresh templates.'
              : 'Add or ingest templates in Design Vault, then refresh this page.'}
          </span>
        </div>
      ) : (
        <div className="template-list vault-template-list">
          {filtered.slice(0, 6).map((design) => {
            const active = selectedSlug === design.slug;
            return (
              <VaultDesignTemplateCard
                key={design.slug}
                design={design}
                active={active}
                onSelect={() => onSelect(active ? null : design.slug)}
              />
            );
          })}
        </div>
...
type VaultPreviewStyle = CSSProperties & {
  '--vault-bg': string;
  '--vault-text': string;
  '--vault-primary': string;
  '--vault-secondary': string;
  '--vault-surface': string;
  '--vault-line': string;
  '--vault-muted': string;
};

function BlankVaultTemplateCard({
  active,
  onSelect,
}: {
  active: boolean;
  ...
  return (
    <button
      type="button"
      className={`template-option vault-template-card vault-template-blank${active ? ' active' : ''}`}
      onClick={onSelect}
      aria-pressed={active}
    >
      <span
        className="vault-template-preview vault-preview-system vault-template-preview-blank"
        style={blankVaultPreviewStyle()}
        aria-hidden="true"
      >
        <span className="vault-preview-topline">
          <span>SFA</span>
          <span>16:9</span>
        </span>
        <span className="vault-preview-title">Blank deck</span>
        <span className="vault-preview-rule" />
        <span className="vault-preview-blocks">
          <span />
          <span />
          <span />
        </span>
      </span>
      <span className="vault-template-card-body">
        <span className="template-option-name">Blank SFA deck</span>
        <span className="template-option-desc">
          Continue without a Vault template. The agent can still turn anything into slides, and you can apply Vault style later.
        </span>
      </span>
    </button>
  );
...
function VaultDesignTemplateCard({
  design,
  active,
  onSelect,
}: {
  ...
  const variant = vaultPreviewVariant(design);
  const style = vaultPreviewStyle(design);
  ...
  return (
    <button
      type="button"
      className={`template-option vault-template-card${active ? ' active' : ''}`}
      onClick={onSelect}
      aria-pressed={active}
    >
      <span
      ...
      >
        {previewImage ? (
          ...
          <>
            <span className="vault-preview-topline">
              <span>{design.sourceHost}</span>
              <span>{design.profile?.confidence ?? 'vault'}</span>
            </span>
            <span className="vault-preview-title">{previewTitle}</span>
            <span className="vault-preview-rule" />
            <span className="vault-preview-side">
              <span />
              <span />
              <span />
            </span>
            ...
            <span className="vault-preview-gate">slide style</span>
          </>
        )}
      </span>
      <span className="vault-template-card-body">
        <span className="vault-template-title-row">
          <span className="template-option-name">{design.title}</span>
          {typeLabel ? <span className="vault-template-kind">{typeLabel}</span> : null}
          <span className="vault-template-swatches" aria-hidden="true">
            {swatches.map((color) => (
              <span key={color} style={{ background: color }} />
            ))}
          </span>
        </span>
        <span className="template-option-desc vault-template-meta">
          {design.sourceHost}
          {design.profile?.archetype ? ` · ${design.profile.archetype}` : ''}
        </span>
        {trait ? (
          <span className="template-option-desc vault-template-summary">
            {trait}
          </span>
        ) : null}
      </span>
...
function blankVaultPreviewStyle(): VaultPreviewStyle {
  return {
    '--vault-bg': '#141312',
    '--vault-text': '#f7f3ee',
    '--vault-primary': '#d97855',
    '--vault-secondary': '#5a514a',
    '--vault-surface': '#211f1d',
    '--vault-line': 'rgba(247, 243, 238, 0.2)',
    '--vault-muted': 'rgba(247, 243, 238, 0.58)',
  };
}

function vaultPreviewStyle(design: VaultDesignMeta): VaultPreviewStyle {
  const colors = vaultColorRoles(design);
  const background = colors.background;
  const text = colors.text || readableTextFor(background);
  const dark = isDarkHex(background);
  return {
    '--vault-bg': background,
    '--vault-text': text,
    '--vault-primary': colors.primary,
    '--vault-secondary': colors.secondary,
    '--vault-surface': colors.surface,
    '--vault-line': colorWithAlpha(text, dark ? 0.28 : 0.18),
    '--vault-muted': colorWithAlpha(text, dark ? 0.62 : 0.54),
  };
}

function vaultColorRoles(design: VaultDesignMeta) {
  ...
  const background = cleanHexColor(
    ...
    cleanHexColor(tokenColors.surface, '#161412'),
  );
  ...
  const primary = cleanHexColor(
    ...
    cleanHexColor(tokenColors.primary, '#d97855'),
  );
  const secondary = cleanHexColor(
    profileColors?.brandSecondary,
    cleanHexColor(tokenColors.secondary ?? tokenColors.neutral, '#8a8076'),
  );
  const surface = cleanHexColor(
    tokenColors.surface,
    isDarkHex(background) ? '#201f1d' : '#f7f3ee',
  );
...
function vaultPreviewSwatches(design: VaultDesignMeta): string[] {
  const colors = vaultColorRoles(design);
...
function vaultPreviewVariant(design: VaultDesignMeta): VaultPreviewVariant {
  ...
  if (/(campaign|advocacy|movement|launch|action|brand)/.test(haystack)) {
    return 'campaign';
  }
  if (/(product|developer|platform|saas|dashboard|tool|workflow|infrastructure)/.test(haystack)) {
    return 'product';
  }
  if (/(editorial|magazine|portfolio|gallery|studio|publication)/.test(haystack)) {
    return 'editorial';
  }
  return 'system';
}

function vaultPreviewTitle(title: string): string {
  const clean = title.replace(/\[[^\]]+\]/g, ' ').replace(/\s+/g, ' ').trim();
  if (/astrodither/i.test(clean)) return 'ASTRO\nDITHER';
...
function vaultTemplateTrait(design: VaultDesignMeta): string {
  const profile = design.profile as Record<string, any> | undefined;
  const guidance = design.profile?.openSlideGuidance;
  const visualDna = profile?.visualDna as Record<string, unknown> | undefined;
  const dnaSignal =
    typeof visualDna?.componentLanguage === 'string'
      ? visualDna.componentLanguage
      : typeof visualDna?.layoutGrammar === 'string'
        ? visualDna.layoutGrammar
        : '';
  return guidance?.direction || dnaSignal || design.summary || '';
}

function vaultTemplateTypeLabel(design: VaultDesignMeta): string {
...
function buildMetadata(input: {
  ...
  if (input.tab === 'deck') {
    return {
      ...
      deliveryOptions: { html: true, pdf: true, pptx: true },
      ...inspirations,
    };
  }
...
function compactVaultComponentMotionRecipes(
  value: VaultDesignMeta['profile'] extends infer Profile
    ? Profile extends { componentMotionRecipes?: infer Recipes }
      ? Recipes
      : unknown
    : unknown,
): NonNullable<ProjectMetadata['vaultTemplate']>['componentMotionRecipes'] {
  if (!Array.isArray(value)) return undefined;
  const recipes = value
    .filter((recipe) => recipe && typeof recipe === 'object' && !Array.isArray(recipe))
    .filter((recipe) => typeof recipe.id === 'string' && recipe.id.trim() && typeof recipe.component === 'string' && recipe.component.trim())
    .slice(0, 6);
  return recipes.length > 0
    ? recipes as NonNullable<ProjectMetadata['vaultTemplate']>['componentMotionRecipes']
    : undefined;
}

export function buildVaultTemplateMetadata(
  design: VaultDesignMeta | null,
): ProjectMetadata['vaultTemplate'] | undefined {
  ...
  return {
    ...
    sourceHost: design.sourceHost || 'Design Vault',
    summary: design.summary,
    tags: design.tags,
    previewImage: design.previewImage,
    manifestPath: design.manifestPath,
    capabilitiesPath: design.capabilitiesPath,
    skillPath: design.skillPath,
    archetype: profile?.archetype,
    confidence: profile?.confidence,
    visualThesis: profile?.visualThesis,
    ...
    narrativeFit: profile?.narrativeFit,
    avoidWhen: profile?.avoidWhen,
    matchingRationale: profile?.matchingRationale,
    slidePatterns: profile?.slidePatterns,
    typographyPersonality: profile?.typographyPersonality,
    layoutIntensity: profile?.layoutIntensity,
    assetNeeds: profile?.assetNeeds,
    localizationFit: profile?.localizationFit,
    colorRoles: profile?.colorRoles,
    typographyRoles: profile?.typographyRoles,
    openSlideGuidance: profile?.openSlideGuidance,
    componentMotionRecipes: compactVaultComponentMotionRecipes(profile?.componentMotionRecipes),
    designPath: design.designPath,
    openSlideThemePath: design.openSlideThemePath,
    tokensPath: design.tokensPath,
    evidencePath: design.evidencePath,
    profilePath: design.profilePath,
    references: design.references,
    activationPrompt: design.activationPrompt,
    previewPpt: design.previews?.ppt,
    previewWeb: design.previews?.web,
  };
}
