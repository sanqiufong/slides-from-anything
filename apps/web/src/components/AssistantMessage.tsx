...
import type { AgentEvent, ChatMessage, ProjectFile, VaultDesignMeta } from '../types';
          ...
          if (b.kind === 'text')
            return (
              <ProseBlock
                ...
                activeVaultTemplateSlug={activeVaultTemplateSlug}
    ...
    <div className="produced-files">
      ...
      <div className="produced-files-list">
        {files.map((f) => {
          ...
          return (
          <div key={f.name} className={`produced-file ${deckSource ? 'openppt-core' : ''}`}>
            <span className="produced-file-icon" aria-hidden>
              <Icon name={deckSource ? 'present' : kindIconName(f.kind)} size={deckSource ? 16 : 14} />
            </span>
            <span className="produced-file-name-wrap" title={f.name}>
              <span className="produced-file-name">{deckLabel?.title ?? f.name}</span>
              {deckLabel ? <span className="produced-file-sub">{deckLabel.subtitle}</span> : null}
            </span>
...
): 'file-code' | 'image' | 'pencil' | 'present' | 'file' {
