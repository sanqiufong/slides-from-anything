...
describe('streamViaDaemon', () => {
  ...
  it('prefers structured agent errors over incidental stderr tails', async () => {
    const handlers = createDaemonHandlers();
    const codexErrorLine = JSON.stringify({
      type: 'error',
      ...
      vi.fn()
        ...
        .mockResolvedValueOnce(
          sseResponse(
            [
              'event: agent',
              `data: ${JSON.stringify({ type: 'raw', line: codexErrorLine })}`,
              '',
              'event: stderr',
              'data: {"chunk":"WARN codex_core::plugins::manifest: ignoring interface.defaultPrompt"}',
              '',
              'event: end',
    ...
    expect(error.message).toContain("The 'gpt-5-codex' model is not supported");
    expect(error.message.indexOf("The 'gpt-5-codex' model is not supported")).toBeLessThan(
      error.message.indexOf('WARN codex_core::plugins::manifest'),
    );
    expect(handlers.onDone).not.toHaveBeenCalled();
  });
