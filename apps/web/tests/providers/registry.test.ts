...
describe('fetchAppUpdateCheck', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    vi.unstubAllGlobals();
  });

  it('returns update check status from the daemon response', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response(JSON.stringify({
        current: {
    ...
    await expect(fetchAppUpdateCheck()).resolves.toMatchObject({
      status: 'latest',
      latestVersion: '1.0.0',
      sourceMode: 'source',
    });
  ...
  it('returns null when update status is malformed', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response(JSON.stringify({ status: 'latest' }), { status: 200 })),
    );

    await expect(fetchAppUpdateCheck()).resolves.toBeNull();
  });
});
...
describe('applyOpenSlideEdits', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    vi.unstubAllGlobals();
  });

  it('returns edit failure context instead of swallowing HTTP failures', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response('target 12:4 not found', { status: 400, statusText: 'Bad Request' })),
    );

    await expect(
      applyOpenSlideEdits('project-1', {
        slideId: 'main-deck',
        edits: [{ line: 12, column: 4, ops: [{ kind: 'set-style', key: 'color', value: 'red' }] }],
      }),
    ).resolves.toEqual({
      ok: false,
      changed: false,
      results: [{ ok: false, error: 'target 12:4 not found' }],
    });
  });
});

describe('fetchVaultDesigns', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    vi.unstubAllGlobals();
  });

  it('keeps different vault template slugs even when they share the same upstream source', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response(JSON.stringify({
        designs: [
          {
            ...
            sourceUrl: 'https://github.com/op7418/guizang-ppt-skill',
          },
          {
            slug: 'guizang-swiss-international',
            title: 'Guizang Swiss International',
            sourceUrl: 'https://github.com/op7418/guizang-ppt-skill',
          },
          {
            slug: 'guizang-swiss-international',
            title: 'Guizang Swiss International duplicate',
            sourceUrl: 'https://github.com/op7418/guizang-ppt-skill',
          },
        ],
      }), { status: 200 })),
    ...
    await expect(fetchVaultDesigns()).resolves.toMatchObject([
      { slug: 'guizang-ppt-skill' },
      { slug: 'guizang-swiss-international' },
    ]);
  });
