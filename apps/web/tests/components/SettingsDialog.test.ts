...
describe('SettingsDialog Codex Image Proxy helpers', () => {
  it('builds OpenAI-compatible image curl commands', () => {
    expect(buildCodexImageProxyCurl('http://127.0.0.1:7456/v1')).toContain(
      'http://127.0.0.1:7456/v1/images/generations',
    );
    expect(buildCodexImageProxyCurl('http://127.0.0.1:7456/v1')).toContain(
      '"model":"gpt-image-2"',
    );
  ...
  it('labels detected Codex auth sources', () => {
    expect(codexImageProxyAuthLabel(null)).toBe('Checking...');
    expect(codexImageProxyAuthLabel({
      enabled: true,
