...
function migrate(db) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_slide_feedback_project
      ON slide_feedback(project_id, updated_at DESC);

    CREATE INDEX IF NOT EXISTS idx_slide_feedback_conversation
      ON slide_feedback(project_id, conversation_id, updated_at DESC);
  ...
  if (!messageCols.some((c) => c.name === 'slide_feedback_attachments_json')) {
    db.exec(`ALTER TABLE messages ADD COLUMN slide_feedback_attachments_json TEXT`);
  }
  if (!messageCols.some((c) => c.name === 'vault_context_attachments_json')) {
    db.exec(`ALTER TABLE messages ADD COLUMN vault_context_attachments_json TEXT`);
  }
  ...
  return db
    .prepare(
      `SELECT id, role, content, agent_id AS agentId, agent_name AS agentName,
              ...
              slide_feedback_attachments_json AS slideFeedbackAttachmentsJson,
              vault_context_attachments_json AS vaultContextAttachmentsJson,
...
export function upsertMessage(db, conversationId, m) {
  ...
  if (existing) {
    db.prepare(
      `UPDATE messages
          SET role = ?, content = ?, agent_id = ?, agent_name = ?,
              ...
              slide_feedback_attachments_json = ?, vault_context_attachments_json = ?,
              produced_files_json = ?,
              started_at = ?, ended_at = ?
        WHERE id = ?`,
    ).run(
      ...
      m.slideFeedbackAttachments ? JSON.stringify(m.slideFeedbackAttachments) : null,
      m.vaultContextAttachments ? JSON.stringify(m.vaultContextAttachments) : null,
    ...
    // comment_attachments_json, slide_feedback_attachments_json,
    // vault_context_attachments_json, produced_files_json, started_at,
    // ended_at, position, created_at.
    db.prepare(
      `INSERT INTO messages
         (id, conversation_id, role, content, agent_id, agent_name,
          run_id, run_status, last_run_event_id, events_json,
          attachments_json, comment_attachments_json, slide_feedback_attachments_json,
          vault_context_attachments_json, produced_files_json, started_at, ended_at,
          position, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      m.id,
...
const SLIDE_FEEDBACK_STATUSES = new Set([
  'queued',
  'applying',
  'needs_review',
  'applied',
...
const SLIDE_FEEDBACK_KINDS = new Set([
  'comment',
  'design-token',
  'inspect-edit',
  'semantic-edit',
...
const SLIDE_FEEDBACK_COLS = `id, project_id AS projectId,
  conversation_id AS conversationId, kind, slide_id AS slideId,
  page_index AS pageIndex, line, column, target_label AS targetLabel,
  note, source, payload_json AS payloadJson, status,
  created_at AS createdAt, updated_at AS updatedAt`;

export function listSlideFeedback(db, projectId, conversationId = null) {
  const rows =
    typeof conversationId === 'string' && conversationId
      ? db
          .prepare(
            `SELECT ${SLIDE_FEEDBACK_COLS}
               FROM slide_feedback
              WHERE project_id = ?
      ...
      : db
          ...
          .all(projectId);
  return rows.map(normalizeSlideFeedback);
}

export function insertSlideFeedback(db, projectId, conversationId, input) {
  const kind = typeof input?.kind === 'string' ? input.kind : 'comment';
  if (!SLIDE_FEEDBACK_KINDS.has(kind)) throw new Error('invalid feedback kind');
  const slideId = cleanRequiredString(input?.slideId, 'slideId');
  const note = cleanRequiredString(input?.note, 'note');
  const status = normalizeSlideFeedbackStatus(input?.status ?? 'queued');
  const now = Date.now();
  const id = typeof input?.id === 'string' && input.id.trim() ? input.id.trim() : randomCommentId();
  db.prepare(
    `INSERT INTO slide_feedback
       (id, project_id, conversation_id, kind, slide_id, page_index, line,
        column, target_label, note, source, payload_json, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    id,
    projectId,
    ...
    kind,
    slideId,
    integerOrNull(input?.pageIndex),
    integerOrNull(input?.line),
    integerOrNull(input?.column),
    typeof input?.targetLabel === 'string' ? input.targetLabel : null,
    note,
  ...
  return getSlideFeedback(db, projectId, id);
}

export function updateSlideFeedbackStatus(db, projectId, id, status) {
  const next = normalizeSlideFeedbackStatus(status);
  const now = Date.now();
  db.prepare(
    `UPDATE slide_feedback
        SET status = ?, updated_at = ?
      WHERE id = ? AND project_id = ?`,
  ).run(next, now, id, projectId);
  return getSlideFeedback(db, projectId, id);
}

export function deleteSlideFeedback(db, projectId, id) {
  const result = db
    .prepare(`DELETE FROM slide_feedback WHERE id = ? AND project_id = ?`)
    .run(id, projectId);
  return result.changes > 0;
}

function getSlideFeedback(db, projectId, id) {
  const row = db
    .prepare(`SELECT ${SLIDE_FEEDBACK_COLS} FROM slide_feedback WHERE id = ? AND project_id = ?`)
    .get(id, projectId);
  return row ? normalizeSlideFeedback(row) : null;
}

function normalizeSlideFeedback(row) {
  return {
    id: row.id,
    projectId: row.projectId,
    ...
    kind: row.kind,
    slideId: row.slideId,
    pageIndex: row.pageIndex == null ? undefined : Number(row.pageIndex),
    line: row.line == null ? undefined : Number(row.line),
    column: row.column == null ? undefined : Number(row.column),
    targetLabel: row.targetLabel ?? undefined,
    note: row.note,
    source: row.source ?? undefined,
    payload: parseJsonOrUndef(row.payloadJson),
    status: row.status,
    createdAt: Number(row.createdAt),
    updatedAt: Number(row.updatedAt),
  };
...
function normalizeSlideFeedbackStatus(value) {
  if (!SLIDE_FEEDBACK_STATUSES.has(value)) throw new Error('invalid feedback status');
  return value;
}
...
function normalizeMessage(row) {
  return {
    ...
    slideFeedbackAttachments: parseJsonOrUndef(row.slideFeedbackAttachmentsJson),
    vaultContextAttachments: parseJsonOrUndef(row.vaultContextAttachmentsJson),
