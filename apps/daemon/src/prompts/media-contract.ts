...
## Auxiliary skill — SFA deck media generation (load-bearing)

This project is still a **Slides from Anything (SFA) Web-PPT deck**. The canonical deliverable
...
### Project metadata to obey

Read the \`deckMedia\` fields in the Project metadata block:

- \`deckMediaImageModel\` is the configured image model when the project has one. If it is absent, no generic default image model is implied; do not assume \`gpt-image-2\` or any other specific provider is available.
- \`deckMediaAspect\` defaults to \`16:9\` when unspecified.
- \`deckMediaKeySlidePolicy\` describes which pages need generated images.
...
### Design-system inheritance for image prompts
...
cues. Do not satisfy the style by color alone.

The daemon also appends a compact "SFA media prompt style context" to image
tasks when it can resolve an active Vault system. Treat that appendix as a
style brief for the provider, not as visible text to put inside the image.

### Slot-first image generation

Write the page layout and Media Slot Plan before dispatching image generation.
Each image prompt must include the intended slide page, output filename,
narrative job, container dimensions or CSS size, final aspect ratio,
fit/crop policy, focal-safe zone, and whether the asset should include negative
space for adjacent typography or chrome. The generated image must be composed
for that exact slot. Do not create a generic image and resize, stretch, or crop
it until it happens to fit the deck.

Use a \`--aspect\` value that matches the planned slot. If the ideal slot ratio
is not supported by the configured model, adjust the slide container to the
nearest supported ratio before generation, or explicitly design a safe crop
inside the prompt. Do not leave the mismatch to CSS after the image exists.
...
textual stand-in for a generated picture in presentation-ready source.

### Required invocation pattern

When \`deckMediaImageModel\` is present, run the dispatcher from your shell
tool for each required generated image:
...
Allowed image model IDs include: ${IMAGE_IDS}.
...
command succeeds, import the asset in \`slides/<slideId>/index.tsx\`, for example:

\`\`\`tsx
import heroImage from './assets/hero-context.png';
// ...
<img src={heroImage} alt="..." className="..." />
\`\`\`

CSS backgrounds are also fine when the image is decorative, but the imported
file must be the real generated asset. If the dispatcher returns a long-running
\`taskId\`, continue with \`"$OD_NODE_BIN" "$OD_BIN" media wait <taskId>\` until
it returns a final \`file\` object or a provider failure.

### Failure behavior

If media generation fails, surface the exact stderr / exit status to the user.
You may still write or keep the deck source with an explicit pending media slot
so layout and narrative work can continue. Mark that region with
