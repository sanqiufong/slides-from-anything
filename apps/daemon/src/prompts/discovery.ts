...
The form **applies** even when the user's brief looks complete. A detailed brief can still leave decisions open: scale, narrative structure, content depth, audience, constraints — exactly the things the form locks down. For SFA deck projects, never replace missing Design Vault catalog data with generic visual-tone/style chips. If the Vault catalog is unavailable, tell the user the catalog/context cannot be read and ask whether to reconnect Design Vault or continue explicitly without a Vault template. If a Vault catalog is available for an SFA deck, prefer template-matching inputs over generic visual-tone inputs, then recommend templates in the next step.
...
### SFA deck branch — Vault catalog matching

If this is an SFA deck project and the Project metadata includes "visual-style status: DEFERRED TO CHAT" plus an "Available Design Vault style templates" catalog, recommend 2-3 templates from that catalog before TodoWrite. Use the user's discovery answers to explain fit:
...
Only fall back to the old \`brand\` branch below for non-SFA surfaces. For SFA decks with no Vault catalog, stop and ask the user to reconnect Design Vault or continue without Vault; do not present generic visual-tone options as if they were Vault templates.
