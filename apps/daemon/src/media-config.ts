...
export function extractAccountIdFromJwt(token) {
  if (typeof token !== 'string' || !token.includes('.')) return '';
  try {
    const payload = token.split('.')[1];
    if (!payload) return '';
    const parsed = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8'));
    const auth = parsed && typeof parsed === 'object'
...
function oauthCredentialFromHermesAuth(data) {
  ...
  if (providerToken) {
    const accountId =
      readNestedString(data, ['providers', 'openai-codex', 'tokens', 'account_id'])
      || extractAccountIdFromJwt(providerToken);
    const refreshToken = readNestedString(data, [
      'providers',
      'openai-codex',
      'tokens',
      'refresh_token',
  ...
  if (Array.isArray(pool)) {
    for (const item of pool) {
      ...
      if (token) {
        return {
          apiKey: token,
          accountId: readNestedString(item, ['account_id']) || extractAccountIdFromJwt(token),
          refreshToken: readNestedString(item, ['refresh_token']),
          source: 'oauth-hermes',
        };
      }
    }
...
function tokenFromHermesAuth(data) {
  return oauthCredentialFromHermesAuth(data)?.apiKey || '';
}
...
function tokenFromCodexAuth(data) {
  ...
  if (oauthToken) {
    return {
      token: oauthToken,
      source: 'oauth-codex',
      accountId: readNestedString(data, ['tokens', 'account_id']) || extractAccountIdFromJwt(oauthToken),
      refreshToken: readNestedString(data, ['tokens', 'refresh_token']),
    };
  }
...
export async function resolveOpenAIOAuthCredential() {
  const home = homedir();
  const hermesAuth = await readJsonIfPresent(
    path.join(home, '.hermes', 'auth.json'),
  );
  const hermesCredential = oauthCredentialFromHermesAuth(hermesAuth);
  if (hermesCredential) {
    return hermesCredential;
  }
  ...
  if (codexToken) {
    return {
      apiKey: codexToken.token,
      source: codexToken.source,
      accountId: codexToken.accountId || '',
      refreshToken: codexToken.refreshToken || '',
    };
  }
