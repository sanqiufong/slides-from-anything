...
import { generateCodexImageWithOAuth } from './codex-image-proxy.js';
import { resolveOpenAIOAuthCredential, resolveProviderConfig } from './media-config.js';
import {
  ensureProject,
  kindFor,
  mimeFor,
...
async function renderOpenAIImage(ctx, credentials) {
  ...
  const codexFallback = await renderCodexImageFallback(ctx, credentials, {
    azure,
    directError: null,
    size: body.size,
    quality: body.quality,
  });
  if (codexFallback) return codexFallback;
  ...
  if (!resp.ok) {
    ...
    const directError = new Error(`${tag} ${resp.status}: ${truncate(text, 240)}`);
    const fallbackAfterDirectFailure = await renderCodexImageFallback(ctx, credentials, {
      azure,
      directError,
      size: body.size,
      quality: body.quality,
    });
    if (fallbackAfterDirectFailure) return fallbackAfterDirectFailure;
...
async function renderCodexImageFallback(ctx, credentials, { azure, directError, size, quality }) {
  if (azure) return null;
  const oauth = await resolveOpenAIOAuthCredential();
  if (!oauth?.apiKey || oauth.apiKey !== credentials.apiKey || !oauth.accountId) {
    return null;
  }

  let data;
  try {
    data = await generateCodexImageWithOAuth({
      credential: oauth,
      prompt: ctx.prompt || 'A high-quality reference image.',
      model: ctx.model,
      n: 1,
    ...
    wrapped.status = typeof err?.status === 'number' ? err.status : 502;
    wrapped.code = err?.code || 'CODEX_IMAGE_BACKEND_FAILED';
    throw wrapped;
  }
  const entry = data && Array.isArray(data.data) ? data.data[0] : null;
  if (!entry) throw new Error('codex image response had no data[0]');
  let bytes;
  if (entry.b64_json) {
    bytes = Buffer.from(entry.b64_json, 'base64');
  } else if (entry.url) {
    const imgResp = await fetch(entry.url);
    if (!imgResp.ok) throw new Error(`codex image fetch ${imgResp.status}`);
    const arr = await imgResp.arrayBuffer();
    bytes = Buffer.from(arr);
  } else {
    throw new Error('codex image response had neither b64_json nor url');
  }

  return {
    bytes,
    providerNote: `codex-backend/${ctx.model} · ${ctx.aspect} · ${bytes.length} bytes`,
    suggestedExt: '.png',
  };
}

function errorWithCause(err) {
  const message = err instanceof Error ? err.message : String(err);
  const cause = err && typeof err === 'object' ? err.cause : null;
  if (cause instanceof Error && cause.message && !message.includes(cause.message)) {
    return `${message} (${cause.message})`;
  }
  if (cause && typeof cause === 'object' && 'code' in cause && !message.includes(String(cause.code))) {
    return `${message} (${String(cause.code)})`;
  }
  return message;
}
