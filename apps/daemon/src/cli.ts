...
startServer({ port, host, returnServer: true }).then(({ url, server }) => {
  // Keep a strong reference in daemon mode. Some launchers start the CLI with
  // stdio detached; retaining the server avoids a quiet process exit after the
  // startup Promise settles.
  globalThis.__openDesignDaemonServer = server;
...
async function runMediaGenerate(rawArgs) {
  ...
  if (!flags.model) {
    console.error(
      '--model required. The media dispatcher has no default image/video/audio model. ' +
        'Choose one from http://<daemon>/api/media/models or, for an SFA deck with no deckMediaImageModel, ' +
        'mark the media slot pending and ask the user to select a model before retrying.',
    );
    process.exit(2);
  }
