...
describe('/api/version', () => {
  ...
  it('returns app update check status', async () => {
    const res = await fetch(`${baseUrl}/api/updates/check`);
    ...
    expect(json).toMatchObject({
      ...
      status: expect.stringMatching(/^(disabled|latest|available|error)$/),
      checkedAt: expect.any(String),
      sourceMode: expect.stringMatching(/^(source|packaged)$/),
