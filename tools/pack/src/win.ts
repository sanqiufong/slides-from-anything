...
const PRODUCT_NAME = "Slides from Anything";
...
LangString OD_REMOVE_LOCAL_DATA_HINT 1033 "Choose whether the uninstaller should remove Slides from Anything data stored on this computer."
LangString OD_REMOVE_LOCAL_DATA_HINT 2052 "请选择卸载程序是否删除此电脑上保存的 Slides from Anything 数据。"
LangString OD_REMOVE_LOCAL_DATA_HINT 1028 "請選擇解除安裝程式是否刪除此電腦上儲存的 Slides from Anything 資料。"
LangString OD_REMOVE_LOCAL_DATA_HINT 1046 "Escolha se o desinstalador deve remover os dados do Slides from Anything armazenados neste computador."
LangString OD_REMOVE_LOCAL_DATA_HINT 1049 "Выберите, должен ли деинсталлятор удалить данные Slides from Anything, сохраненные на этом компьютере."
LangString OD_REMOVE_LOCAL_DATA_HINT 1065 "انتخاب کنید که حذف‌کننده داده‌های Slides from Anything ذخیره‌شده در این رایانه را حذف کند یا نه."

LangString OD_REMOVE_LOCAL_DATA_CHECKBOX 1033 "Remove local Slides from Anything data:"
LangString OD_REMOVE_LOCAL_DATA_CHECKBOX 2052 "删除本地 Slides from Anything 数据："
LangString OD_REMOVE_LOCAL_DATA_CHECKBOX 1028 "刪除本機 Slides from Anything 資料："
LangString OD_REMOVE_LOCAL_DATA_CHECKBOX 1046 "Remover dados locais do Slides from Anything:"
LangString OD_REMOVE_LOCAL_DATA_CHECKBOX 1049 "Удалить локальные данные Slides from Anything:"
LangString OD_REMOVE_LOCAL_DATA_CHECKBOX 1065 "حذف داده‌های محلی Slides from Anything:"
...
!macro customUnInstall
  ...
  \${If} $odRemoveLocalData != "0"
    DetailPrint "Removing local Slides from Anything data: $odLocalDataRoot"
...
async function writeAssembledApp(config: ToolPackConfig, paths: WinPaths, packedTarballs: PackedTarballInfo[]): Promise<void> {
  ...
  await writeFile(
    ...
    `${JSON.stringify(
      {
        dependencies,
        description: "Slides from Anything packaged runtime",
        main: "./main.cjs",
