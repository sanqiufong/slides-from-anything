...
const DEFAULT_LOCAL_NO_PROXY = "localhost,127.0.0.1,::1";
...
function withDefaultLocalNoProxy(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  if (!hasProxyEnv(env) || hasNoProxyEnv(env)) return env;
  return { ...env, NO_PROXY: DEFAULT_LOCAL_NO_PROXY };
}

function withNodeProxyEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const base = hasProxyEnv(env) ? env : { ...macOsSystemProxyEnv(), ...env };
  if (!hasProxyEnv(base)) return base;
  return {
    ...base,
    NODE_USE_ENV_PROXY: base.NODE_USE_ENV_PROXY || "1",
  };
}

function macOsSystemProxyEnv(): NodeJS.ProcessEnv {
  if (process.platform !== "darwin") return {};
  let raw = "";
  try {
    raw = execFileSync("scutil", ["--proxy"], {
      encoding: "utf8",
      timeout: 1000,
    });
  } catch {
  ...
  const httpProxy = proxyUrlFromScutil(fields, "HTTP");
  const httpsProxy = proxyUrlFromScutil(fields, "HTTPS");
  if (httpProxy) env.HTTP_PROXY = httpProxy;
  if (httpsProxy) env.HTTPS_PROXY = httpsProxy;
  return env;
}

function proxyUrlFromScutil(fields: Map<string, string>, prefix: "HTTP" | "HTTPS"): string | null {
  if (fields.get(`${prefix}Enable`) !== "1") return null;
  const host = fields.get(`${prefix}Proxy`);
  const port = fields.get(`${prefix}Port`);
...
async function spawnSidecarRuntime(request: {
  ...
  const spawned = await spawnBackgroundProcess({
    ...
    detached: true,
    env: withDefaultLocalNoProxy(withNodeProxyEnv({
      ...process.env,
      ...env,
      ...request.env,
    })),
...
async function spawnDesktopRuntime(config: ToolDevConfig, options: CliOptions): Promise<{ pid: number }> {
  ...
  try {
    ...
    const spawnEnv: NodeJS.ProcessEnv = withDefaultLocalNoProxy({
      ...process.env,
      ...env,
      ...(options.parentPid == null ? {} : { [TOOLS_DEV_PARENT_PID_ENV]: String(options.parentPid) }),
    });
